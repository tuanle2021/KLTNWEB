...
export const addBrand = createAsyncThunk(
    ...
    async (brandData, { rejectWithValue, getState }) => {
        try {
            const state = getState();
            const token = state.auth.user.token;
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/brands`, brandData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
...
export const updateBrand = createAsyncThunk(
    ...
    async ({ id, brandData }, { rejectWithValue, getState }) => {
        try {
            const state = getState();
            const token = state.auth.user.token;
            const response = await axios.put(`${process.env.REACT_APP_BACKEND_URL}/brands/${id}`, brandData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
