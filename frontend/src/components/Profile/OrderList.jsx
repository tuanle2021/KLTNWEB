...
const OrderListComponent = ({ title, orders }) => {
    ...
    const handleReviewSubmit = async (productId) => {
        try {
            ...
            setReviewingProduct(null); // Close review form after successful submission
        } catch (error) {
                    ...
                    <ActionsCell>
                        {row.original.status === "shipped" && (
                            <Button
                                ...
                                onClick={() =>
                                    setReviewingProduct(row.original.items[0].product_id._id)
                                }
                            >
                                Review
                            </Button>
                        ...
                        {row.original.status !== "cancelled" && (
                            <Button
                                className="map-btn"
                                onClick={() => {
                                    setExpandedMapIndex(expandedMapIndex === row.index ? null : row.index);
                                    console.log("Start Coordinates:", orderCoordinates[row.index]?.startCoordinates);
                                    console.log("End Coordinates:", orderCoordinates[row.index]?.endCoordinates);
                                }}
                            >
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={columns.length}>
                                            <div style={{ height: "300px" }}>
                                                <MapComponent
                                                    startCoordinates={orderCoordinates[row.index]?.startCoordinates}
                                                    endCoordinates={orderCoordinates[row.index]?.endCoordinates}
                                                />
                                            </div>
                                        </TableCell>
                                    </TableRow>
